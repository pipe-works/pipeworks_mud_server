...
``types.py``
  ...
  ``ActivationScope``, ``EffectiveAxisBundle``, and ``EffectiveImagePolicyBundle``.
...
``runtime_resolution.py``
  Owns runtime effective resolution for policy variants, prompt templates, axis bundles,
  and image-policy diagnostic bundles.
...
``paths.py``
  Owns policy export root resolution used by artifact bootstrap/import flows.
...
* ``resolve_effective_image_policy_bundle``
* ``publish_scope``
* ``get_publish_run``
...
2. Integration coverage for policy API routes and retained CLI commands.
