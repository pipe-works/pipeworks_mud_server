...
@pytest.mark.unit
def test_engine_bootstrap_axis_policy_skips_when_canonical_activation_missing(
    caplog, monkeypatch
) -> None:
    """Missing canonical activation should log guidance and skip registry seeding."""
    with patch.object(GameEngine, "__init__", lambda self: None):
        engine = GameEngine()
    cast(Any, engine).world_registry = SimpleNamespace(
    ...
    def _fail_seed_axis_registry(**_kwargs):
        calls["seed"] += 1
        raise AssertionError(
            "seed_axis_registry should not run when canonical activation is missing."
        )

    monkeypatch.setattr(
        "mud_server.services.policy_service.resolve_effective_axis_bundle", _resolve
    )
    monkeypatch.setattr(database, "seed_axis_registry", _fail_seed_axis_registry)
    ...
    assert calls["seed"] == 0
    assert any(
        "no effective canonical axis/manifest activation was found" in message
